...
% DRAFT, 2026-09-25: Chris's idea (fireball rocket jump as the model
% case for GM discretion). Decided: the tone is a table-wide dial, set
% before play; under Grounded, a stunt's side effect joins the stakes as
% benefit on success, harm on failure, both on a partial.
...
\begin{itemize}
  \item \keyword{Rule of Cool}: stunts work the way the player pictures
    ...
    caster falls short and pays the ordinary cost.
...
\begin{example}
\emph{The rocket jump.} A wizard needs to cross a chasm and has no
bridge, so she fires a fireball at her own feet. Gravity and distance
resist, so it's a \keyword{Body} roll; the GM sets a target of \dice{2}.
Under Rule of Cool, it's an ordinary jump: on a success she sails
across, and on a failure she comes up short and scrambles for a handhold.
Under Grounded, the GM names the harm before the roll: \dice{1} Body
from the blast. On a success she lands clean. On a partial, she lands on
the far side, singed, and loses \dice{1} Body. On a failure, she takes
the burn \emph{and} comes up short.
\end{example}
...
\begin{example}
\emph{The open channel.} Aboard a starship, an engineer splices into a
pirate vessel's intercom and plays their captain's private negotiations
to his whole crew. What resists is the crew's loyalty, so it's a
\keyword{Face} roll at \dice{2}, just as a fireball over the guards'
heads is. Under Rule of Cool, the crew mutinies. Under Grounded, the
link runs both ways, and the harm is that the pirates trace it back to
the party's ship. Harm doesn't have to mean lost points: on a partial,
the crew turns on its captain but the rest of the pirate fleet now
knows exactly where the party is.
\end{example}
